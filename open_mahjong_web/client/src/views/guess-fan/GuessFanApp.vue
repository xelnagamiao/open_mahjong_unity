          ...
          <div class="guide-grid">
            <article>
              ...
              <ul class="related-cases">
                <li><b>同义、同名关联：</b>两个规则中的同名番，或共享同一种番型的同义番。例如答案是立直「一气通贯」，猜国标「清龙」，名字显示黄色；只有猜中题库中的那个具体番种才会绿色。<br />示例 1：答案一气通贯 [一气通贯、清龙]，猜出清龙 [清龙、一气通贯] 时名字一侧会显示关联的黄色提示。<br />示例 2：答案平和 [国标]，猜测时选择平和 [日麻]，同名的平和一侧会显示关联的黄色提示。<br />示例 3：答案全大 [全大，全中，全小]，猜出全中时名字一侧会显示关联的黄色提示；只有猜中全大本身才绿色。</li>
              ...
              <p class="related-off"><b>关闭关联提示：</b>所有上述黄色提示都会变成灰色，只保留完全匹配的绿色和数值方向箭头，提升对局难度。</p>
            </article>
            <article>
...
const scoreText = computed(() => {
  ...
  const me = players.find((p) => p.id === myId.value)
  const opponent = players.find((p) => p.id !== myId.value)
  return `${me?.nick || '?'} ${room.value.scores?.[me?.id] || 0}:${room.value.scores?.[opponent?.id] || 0} ${opponent?.nick || '?'}`
})
