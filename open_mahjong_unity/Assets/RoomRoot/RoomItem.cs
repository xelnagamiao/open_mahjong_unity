...
public class RoomItem : MonoBehaviour
{
    ...
    [SerializeField] private Text roomName; // 房间名
    [SerializeField] private Text hostName; // 房主名
    [SerializeField] private Text playerNum; // 玩家人数
    [SerializeField] private Text roomNum; // 房间号
    [SerializeField] private Text gameTime; // 游戏时间
    [SerializeField] private Button joinButton; // 加入按钮
    [SerializeField] private Text hasPassword; // 是否有密码
    ...
    public void SetRoomListInfo(string roomId,string roomName,string hostName,int playerCount,int gameTime,bool hasPassword)
    {
        ...
        this.roomNum.text = $"房间号: {roomId}";
        this.roomName.text = $"房间名: {roomName}";
        this.hostName.text = $"房主: {hostName}";
        this.playerNum.text = $"{playerCount}/4";
        this.gameTime.text = $"圈数：{gameTime}";
