...
public partial class GameRecordManager {
    ...
    private void CompleteRecordAutoPlay(bool showCompletedTip = true) {
        ...
        if (wasPlaying && showCompletedTip) {
            GameHost.Current.ShowTip("牌谱", true, "自动播放完毕");
        }
    }
    ...
    public void NextXunmu() {
        ...
        // 无下一巡时，优先在和牌/流局节点停顿一次，避免直接跨到 end
    ...
    private static readonly HashSet<string> XunmuPauseActionKeys = new HashSet<string> {
        ...
        "liuju", "ryuukyoku", "shuhewei"
    };

    private bool IsTerminalPauseAction(string action) {
        if (string.IsNullOrEmpty(action) || !XunmuPauseActionKeys.Contains(action)) return false;
        if (RecordHuTickFollowsShuhewei() && IsClassicalFollowSettlementAction(action)) return false;
        return true;
    }
    ...
    private int FindSettlementPauseNodeAtOrAfter(Round roundData, int nodeIndex) {
        ...
        for (int i = start; i < roundData.actionTicks.Count; i++) {
            List<string> tick = roundData.actionTicks[i];
            if (tick == null || tick.Count == 0) continue;
            if (IsTerminalPauseAction(tick[0])) return i;
        }
        return -1;
    }
