Shader "Custom/ThreeDTiles"
{
    Properties
    {
        ...
        // 与 TileTextureLayout.TableAspect 一致；此值由 CardBackManager 同步。
        _TableFaceAspect ("Table Face UV Canvas Aspect (w/h)", Float) = 0.7518797
        _TableFaceImageScale ("Complete Image Area Scale", Range(0.1, 1)) = 0.86
        _FrontTexContain ("Standalone Front Contain (0=atlas 1=complete image)", Float) = 0
        // 3D 牌面背景铺满模式：0=完整等比居中 1=铺到整张牌面+侧面边缘（拉伸覆盖）。
        ...
        // Matte cel shading: two albedo-multiplied tones, without additive
        // highlights, reflections, or a separately brightened bevel/face mask.
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 Frag(Varyings input) : SV_Target
            {
                ...
                float2 frontUV =
                    imageUV * frontTilingOffset.xy + frontTilingOffset.zw;
                ...
                // Contain 会产生越界 UV。以透明处理留白，不能复制 Clamp 边缘像素。
                half frontInside = step(0.0, frontUV.x) * step(frontUV.x, 1.0)
                    * step(0.0, frontUV.y) * step(frontUV.y, 1.0);
                front.a *= imageInside * lerp(1.0h, frontInside, saturate(_FrontTexContain));
                half frontAlpha = saturate(front.a);
                if (saturate(_FrontBgBlend) > 0.0h)
                {
                    float2 frontBgUV = input.uvFront;
                    if (_FrontRotation != 0.0h)
                    {
                    ...
                    {
                        // 在旋转后的纹理宽/高轴上与前景采用相同的完整等比居中。
                        float faceAspect = max(_TableFaceAspect, 0.001);
                        float texAspect = _FrontBgTexAspect;
                        if (texAspect > 0.0)
                        {
                            if (texAspect > faceAspect)
                            {
                                frontBgUV.y = (frontBgUV.y - 0.5) * (texAspect / faceAspect) + 0.5;
                            }
                            else if (texAspect < faceAspect)
                            {
                                frontBgUV.x = (frontBgUV.x - 0.5) * (faceAspect / texAspect) + 0.5;
                            }
                        }
                        bgInside = step(0.0, frontBgUV.x) * step(frontBgUV.x, 1.0)
                            * step(0.0, frontBgUV.y) * step(frontBgUV.y, 1.0);
                    }
                    // 铺满模式（_TableBgCoverFace=1）：直接用整张 UV，覆盖到整张牌面 + 侧面边缘。
                    half4 bgSample = SAMPLE_TEXTURE2D(_FrontBgTex, sampler_FrontBgTex, frontBgUV);
                    half bgAlpha = bgSample.a * saturate(_FrontBgBlend) * bgInside;
                    half3 bgTinted = bgSample.rgb * _FrontBgColor.rgb * frontColor.rgb;
                    half3 bgRgb = lerp(_TableFaceFallbackColor.rgb * frontColor.rgb, bgTinted, bgAlpha);
                    half3 composedRgb = lerp(bgRgb, front.rgb, frontAlpha);
                    half composedA = max(frontAlpha, 0.5h);
                ...
                else if (max(max(saturate(_TableFaceFallbackEnabled), saturate(_FrontTexContain)), 1.0h - imageInside) > 0.5h)
                {
                ...
                // ordinary normals and receive no separate color multiplier.
                ...
                // This stays monotone at stronger settings, unlike multiplying
                // all of x by a brightness-dependent darkening factor. Dark
                // artwork and saturated colors keep their original response.
                half highChannel = max(col.r, max(col.g, col.b));
                half lowChannel = min(col.r, min(col.g, col.b));
                half neutralWhite = saturate(1.0h - (highChannel - lowChannel) * 5.0h);
                half whiteExcess = max(lowChannel - 0.55h, 0.0h);
                half reduction = saturate(_TileWhiteCompression * (1.0h / 0.45h))
                    * whiteExcess * neutralWhite;
                col.rgb *= 1.0h - reduction / max(lowChannel, 0.0001h);
        ...
        // Draw only the back faces of a screen-expanded hull. This pass reuses
        // the mesh and camera depth; it needs no ID/mask/depth texture sampling.
        Pass
        {
            ...
            float _TileHullOutlineWidth;

            struct HullAttributes
            {
                float4 positionOS : POSITION;
                float3 normalOS : NORMAL;
                float2 outlineNormalOct : TEXCOORD3;
                UNITY_VERTEX_INPUT_INSTANCE_ID
            };
            struct HullVaryings
            {
            ...
            // UV4 stores an octahedral smooth normal in Unity object space.
            // Sharing it across the lighting splits keeps the expanded hull closed.
            float3 DecodeOutlineNormal(float2 encoded)
            {
            ...
            HullVaryings HullVert(HullAttributes input)
            {
                ...
                float3 normalWS = TransformObjectToWorldNormal(DecodeOutlineNormal(input.outlineNormalOct));
                float4 positionCS = TransformWorldToHClip(positionWS);
                float3 normalVS = TransformWorldToViewDir(normalWS);
                float4 directionCS = mul(UNITY_MATRIX_P, float4(normalVS, 0));
                // Differentiate perspective divide, then normalize in pixel
                // coordinates so portrait/landscape and distance keep line width.
                float2 directionNDC = directionCS.xy - positionCS.xy / max(positionCS.w, 0.00001) * directionCS.w;
                float2 pixelDirection = directionNDC * _ScaledScreenParams.xy;
                pixelDirection *= rsqrt(max(dot(pixelDirection, pixelDirection), 0.000001));
                positionCS.xy += pixelDirection * (2.0 * _TileHullOutlineWidth / _ScaledScreenParams.xy) * positionCS.w;
