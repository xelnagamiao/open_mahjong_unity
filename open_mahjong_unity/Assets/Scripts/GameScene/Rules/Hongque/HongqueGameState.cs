...
/// <summary>
/// 虹雀的族 GameState，对应服务端 server/gamestate/game_hongque。
///
/// 虹雀不走回合制的 ask/do 询问式协议，而是开局/重连发全量桌面、对局中只发增量事件与当前可选项，
/// 所以它不继承 TurnBasedGameState，而是直接实现 IGameState 的三件事：
/// - 入站：game_start / reconnect / update / ready_status 全部由本类消费，投影到同一张牌桌
///   （TableMirror + Game3DManager + GameCanvas），不另建状态；
/// - 出站：按钮点击与出牌走 gamestate/hongque/action 通道；
/// - 生命周期：结算确认以自己的协议发 ready，退出时清空本局游标。
///
/// 动作词（hongque_win / hongque_group:* / hongque_claim:* …）怎么归类、怎么展示，见 HongqueActionWords。
/// 协程通过 GameSession.Current.Host 挂在场景宿主上，本类不是 MonoBehaviour。
/// </summary>
public sealed class HongqueGameState : GameStateBase {
    public const string RuleId = "hongque";

    /// <summary>当前对局若是虹雀则返回其 GameState，否则 null。</summary>
    public static HongqueGameState Active => RuleRegistry.ActiveGameState as HongqueGameState;

    /// <summary>当前是否有一张虹雀牌桌处于活动状态（供词表等静态查询）。</summary>
    public static bool IsTableActive => Active != null && Active.state != null;

    public override string StateId => "hongque.state_sync";
    ...
    public override bool HandleMessage(string suffix, Response response) {
        switch (suffix) {
            ...
            case "reconnect":
            case "update":
                ApplyState(response.gamestate_id, response.hongque_state);
    ...
    public override bool TryChooseAction(string actionType) {
        return IsActive && TryChooseEncodedAction(actionType);
    }

    public override bool TryCutTile(int tileId) {
        if (!IsActive || !HongqueTileVisual.IsHongqueId(tileId)) return false;
        SendDiscard(tileId);
        return true;
    }

    public override bool TryConfirmRoundResult() {
        if (!IsActive || !IsRoundEnd) return false;
        ConfirmRoundResult();
        return true;
    }

    /// <summary>切牌预览：按"打出该牌后的手牌"本地算虹雀听牌/番数提示。</summary>
    public override bool TryShowCutPreviewTips(int cutTileId, bool canShow) {
        if (!IsActive || TipsContainer.Instance == null) return false;
        TableMirror mirror = TableMirror.Current;
        List<int> handAfterDiscard = new List<int>(mirror.SelfHandTiles);
        bool removed = handAfterDiscard.Remove(cutTileId);
        HongqueScoreHintInfo[] localHints = removed
            ? HongqueTenpai.BuildScoreHints(handAfterDiscard, mirror.Self.combination_masks, cutTileId)
            : Array.Empty<HongqueScoreHintInfo>();
        if (!canShow) return true;
        if (localHints.Length > 0) {
            TipsContainer.Instance.SetHongqueCutPreviewHints(localHints, cutTileId);
    ...
    private void SendDiscard(int tileId) {
        if (!IsActive) return;
        // 点击出牌即结束本地操作窗口。不能等服务端 discard 帧回来后再清理，
        // 否则倒计时协程会在网络往返期间短暂刷新出旧值或默认值。
    ...
    private bool TryChooseEncodedAction(string encodedAction) {
        ...
        else if (encodedAction.StartsWith(HongqueActionWords.KongPrefix)) {
            Send("kong", null, encodedAction.Substring(HongqueActionWords.KongPrefix.Length));
        }
        else return false;
    ...
    /// <summary>当前窗口的全部有效候选（带 id）。kind 为 null 时不按种类过滤。</summary>
    public HongqueCandidateInfo[] GetCandidates(string kind = null) {
        return state?.candidates?
            .Where(item => item != null && !string.IsNullOrEmpty(item.id) && (kind == null || item.kind == kind))
    ...
    private void ApplyAvailableActions() {
        ...
        if (legalActions.Contains("win")) actions.Add(HongqueActionWords.Win);
        if (legalActions.Contains("supplement")) actions.Add(HongqueActionWords.Supplement);
        if (state.candidates != null) {
            if (legalActions.Contains("kong") && state.candidates.Any(item => item?.kind == "kong")) {
                actions.Add(HongqueActionWords.GroupPrefix + "kong");
            }
            if (legalActions.Contains("claim")) {
                HongqueCandidateInfo ron = state.candidates.FirstOrDefault(item => item?.kind == "win");
                if (ron != null) actions.Add(HongqueActionWords.ClaimPrefix + ron.id);
                foreach (string kind in new[] { "rainbow", "triplet", "sequence" }) {
                    if (state.candidates.Any(item => item?.kind == kind)) actions.Add(HongqueActionWords.GroupPrefix + kind);
                }
            }
        }
        ...
        if (actions.Count > 0) {
            ...
            // resolving 是出牌事件与鸣牌询问之间的中间帧；claim 无操作表示本家
            // 已经回答。两者都只能清理输入，不能用 AskHandAction(0, ...)
            // 制造一个“0+步时”倒计时，否则会在出牌后闪出默认数字再消失。
            gsm.SwitchCurrentPlayer("self", "ClearAction", 0);
        } else {
            // 真正 turn 阶段且轮到其它家时，用通用入口更新行动者指示；
            // SwitchCurrentPlayer 会走非本家分支并保持计时器关闭。
            TableMirror.Current.RemainTiles = state.wall_count;
            gsm.AskHandAction(0, state.current_player, Array.Empty<string>());
        }
    }
    ...
    private void ApplyRuleTips() {
        ...
        HongqueScoreHintInfo[] waits = Array.Empty<HongqueScoreHintInfo>();
    ...
    private void ApplyIncrementalEvent(HongqueEventInfo actionEvent) {
        ...
        if (actionEvent.type == "discard") {
            ...
            lastDiscardPlayerIndex = actionEvent.player;
            gsm.DoAction(
        ...
        NormalizeHongqueMeldMasks(actionEvent.player);
        if (actionEvent.player == state.you) selfHasUnmergedDraw = false;
        if (rainbow && !claimApplied
                && gsm.indexToPosition.TryGetValue(actionEvent.player, out string position)) {
            GameCanvas.Instance.ShowActionDisplay(position, HongqueActionWords.Rainbow, RuleId);
        }
    }
    ...
    private void ApplyKongEvent(HongqueEventInfo actionEvent) {
        ...
        playerInfo.combination_masks[actionEvent.meld_index] = ToVerticalMeldMask(mask.ToArray());
        ...
        for (int i = 0; i < addedCodes.Length; i++) {
            ...
            // 虹雀杠不走通用“加杠后摸牌”流程，但桌面 3D 手牌仍须实减一张并收拢。
            Game3DManager.Instance.RemoveHongqueKongHandTile(position, tileId);
        }
        if (position == "self") {
            playerInfo.hand_tiles_count = gsm.selfHandTiles.Count;
        }
    ...
    private static int[] ToVerticalMeldMask(int[] mask) {
        if (mask == null) return Array.Empty<int>();
        int[] vertical = (int[])mask.Clone();
    ...
    private static void NormalizeHongqueMeldMasks(int playerIndex) {
        NormalGameStateManager gsm = NormalGameStateManager.Instance;
        if (gsm == null || !gsm.indexToPosition.TryGetValue(playerIndex, out string position)
                || !gsm.player_to_info.TryGetValue(position, out PlayerInfoClass playerInfo)
                || playerInfo.combination_masks == null) {
            return;
        }
        for (int i = 0; i < playerInfo.combination_masks.Count; i++) {
            playerInfo.combination_masks[i] = ToVerticalMeldMask(playerInfo.combination_masks[i]);
        }
        // 普通吃碰已经由 DoAction 排入通用副露动画；这里只归一化计分板使用的竖立标记。
        // 若此处立即重建 3D 副露，会先显示最终位置，再由已排队的动画重复生成并移动一次。
    }
    ...
    private void ConfirmRoundResult() {
    ...
    private void ShowRoundResult(HongqueStateInfo previous, HongqueStateInfo current) {
        ...
        TipsBlock.Instance?.HideTipsBlock();
        TipsContainer.Instance?.HideTips();
        RecordScoreboardSettlements(current, scores);
        ...
        if (current.round_result.multi_ron && current.round_result.winners.Length > 1) {
            ...
            Host.StartCoroutine(ShowMultiRonSequence(current, scores));
            return;
        }
        ...
        PresentHuResult(result, scores, current, finalPanel: true);
    }

    private static void RecordScoreboardSettlements(
        HongqueStateInfo current, Dictionary<int, int> finalScores) {
        NormalGameStateManager gsm = NormalGameStateManager.Instance;
        if (gsm == null || current?.round_result == null) return;
        HongqueWinnerResultInfo[] winners = current.round_result.winners
            ?? Array.Empty<HongqueWinnerResultInfo>();
        if (winners.Length == 0) {
            gsm.AppendExternalRoundSettlementSnapshot(
                -1, finalScores, 0, Array.Empty<string>(), "liuju",
                Array.Empty<int>(), Array.Empty<int[]>(), null,
                new Dictionary<int, int>());
            return;
        }
        // 服务端 score_history 按整局只追加一行；多家和的逐家面板仍分别展示，
        // 但计分板必须合并成同一局的一行，否则下一局同步时行数不一致会清空快照。
        int primaryWinnerIndex = current.round_result.winner_indices?.FirstOrDefault()
            ?? winners.FirstOrDefault(item => item != null)?.player
            ?? -1;
        HongqueWinnerResultInfo result = winners.FirstOrDefault(
            item => item != null && item.player == primaryWinnerIndex)
            ?? winners.FirstOrDefault(item => item != null);
        if (result == null) return;
        string huClass = current.round_result.reason == "self_draw" ? "hu_self" : "hu";
        Dictionary<int, int> roundChanges = current.round_result.score_changes != null
            ? new Dictionary<int, int>(current.round_result.score_changes)
            : new Dictionary<int, int>();
        gsm.AppendExternalRoundSettlementSnapshot(
            result.player, finalScores, result.points, BuildFanTokens(result), huClass,
            ConvertTiles(result.hand), BuildResultMeldMasks(result.melds),
            result.@base, roundChanges);
    }
    ...
    private IEnumerator ShowMultiRonSequence(HongqueStateInfo current, Dictionary<int, int> finalScores) {
        HongqueWinnerResultInfo[] results = current.round_result.winners;
        ...
        for (int i = 0; i < order.Length; i++) {
            ...
            for (int j = i + 1; j < order.Length; j++) {
                int laterPlayer = order[j];
                int laterPoints = current.round_result.score_changes != null
                    && current.round_result.score_changes.TryGetValue(laterPlayer, out int p)
                    ? p : 0;
                if (stageScores.ContainsKey(laterPlayer)) {
    ...
    private IEnumerator PresentHuResultCoroutine(
        ...
        yield return RoundEndPresentation.Instance.PresentHuResultSequenceCoroutine(
    ...
    private PlayerInfo BuildPlayerInfo(HongquePlayerInfo source, HongqueEventInfo excludedOpeningDraw = null) {
        ...
        return new PlayerInfo {
            ...
            score_history = source.score_history ?? Array.Empty<string>(),
            round_number_history = source.round_number_history ?? Array.Empty<int>(),
