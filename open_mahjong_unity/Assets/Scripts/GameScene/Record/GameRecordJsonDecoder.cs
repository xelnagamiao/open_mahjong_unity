...
public static class GameRecordJsonDecoder {
    ...
    /// <summary>
    /// 从单条 tick 累计本局 score_changes（hu_* / hu_riichi / ryuukyoku，以及川麻 gs/gr/终局分步）。
    /// 供牌谱初次解析与观战增量解析共用，确保跨局起手分不为 0。
    /// </summary>
    public static void AccumulateScoreChangesFromTick(Round round, List<string> tick) {
        ...
        if (tick.Count >= 5 && (act == "hu_self" || act == "hu_first" || act == "hu_second" || act == "hu_third")) {
            ...
            else if (step == "cha_refund") sc = ParseScoreChangesFromTick(tick, 2);
        }
        if (sc == null) return;
        AccumulateScoreChanges(round, ConvertPlayerIndexScoreChangesToOriginal(sc, round.seats));
    }

    /// <summary>川麻刮风下雨：tick 内 <c>gs</c> 后四家分变（按 player_index）。全 0 返回 null。</summary>
    public static int[] ParseInlineGangScoreChanges(List<string> tick) {
        if (tick == null) return null;
        int gsIndex = tick.LastIndexOf("gs");
        if (gsIndex < 0 || gsIndex + 4 >= tick.Count) return null;
        var arr = new int[4];
        for (int i = 0; i < 4; i++) {
            if (!int.TryParse(tick[gsIndex + 1 + i]?.Trim(), out arr[i])) return null;
        }
        if (arr[0] == 0 && arr[1] == 0 && arr[2] == 0 && arr[3] == 0) return null;
        return arr;
    }
    ...
    public static bool ParseBuhuaMoFlag(List<string> tick) {
        // 旧牌谱只有 [bh, tile, player]：默认手补。现行格式第四段 T=摸补 / F=手补。
        if (tick == null || tick.Count < 4 || string.IsNullOrEmpty(tick[3])) {
            return false;
        }
