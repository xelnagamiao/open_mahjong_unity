...
public static class GameRecordMeldCodec {
    ...
    /// <summary>
    /// 暗杠 tick 解析并删牌：现行格式 [ag, norm, T|F, id0..id3]（可另附 gs 等延伸段）。
    /// 旧牌谱缺少真实 ID 时按归一化从手牌删 4 张。
    /// </summary>
    public static List<int> ResolveAngangRemovedTiles(
        ...
        if (tick == null || tick.Count < 3 + AngangHandTileCount) {
            // 旧牌谱无 4 张真实 ID：按归一化从手牌删 4 张，避免跳转/步进直接抛错。
    ...
    public static int[] BuildAngangMaskFromRemoved(IReadOnlyList<int> removedTiles, string rule) {
        ...
        if (!string.IsNullOrEmpty(rule) && RuleRegistry.Resolve(rule, rule)?.FaceDownAnkan == true) {
            return new[] { 0, r0, 0, r1, 0, r2, 0, r3 };
        }
        if (RuleRegistry.Resolve(rule, rule)?.PeekAnkan == true) {
    ...
    public static List<int> ResolveHandTiles(IReadOnlyList<string> tick, string recordAction, int mingpaiTileId) {
        ...
        // 旧牌谱无手牌真实 ID：按被鸣牌归一化推断（赤 5 可能对不齐，仅用于回放兼容）。
        var fallback = new List<int>(expected);
        for (int i = 0; i < expected; i++) {
            fallback.Add(mingpaiTileId);
        }
        return fallback;
    }
