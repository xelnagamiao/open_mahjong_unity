...
> **口径修正（2026-08-05）**：本组主番统计基于 PR #98 误加“隔步（跨 2）”三色三步高的检番口径，**不代表现行国标规则**。国标 MCR 三色三步高仅允许依次递增一位（连步）；隔步误改已回退（服务端 `guobiao_hepai_check.py`、Web `gbHepai.ts` 恢复仅连步，Unity 端从未改动）。按现行规则该热度约为 **24%**，接近人类参考 ~25.5%，不再“过热”。
