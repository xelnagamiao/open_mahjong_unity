...
class HePaiCheck:
    ...
    yaojiuke_set = {"k11","K11","k19","K19","k21","K21","k29","K29","k31","K31","k39","K39",
                        "k41","K41","k42","K42","k43","K43","k44","K44",
                        "g11","G11","g19","G19","g21","G21","g29","G29","g31","G31","g39","G39",
                        "g41","G41","g42","G42","g43","G43","g44","G44"} # 幺九刻
    # 存储排斥的番种
    repel_model_dict:Dict[int,list] ={
        ...
        "yiseshuanglonghui":["qingyise","pinghe","wuzi","yibangao","yibangao"], "yisesitongshun":["siguiyi"], # 一色双龙会 一色四同顺
    ...
    def fan_count_combination_sign_check(self,player_tiles:PlayerTiles,combination_str,way_to_hepai):
        ...
        if len(save_dazi_sign) >= 2:
            ...
            for set in sanseshuanglonghui_list:
                if all(i in save_dazi_sign for i in set):
                    if f"q{save_quetou_sign[0]}" in set:
    ...
    def fan_count(self, player_tiles: PlayerTiles,get_tile,way_to_hepai):
        ...
        self.fan_count_output(player_tiles,combination_str,zimo_or_not)
...
if __name__ == "__main__":
    ...
    # 测试数据 摘自新编MCR番种定义
    # 大四喜
    # 3 test_save = [["k42","k43"],[34,34,41,41,41,42,42,42],42,["点和"]]
    # 4 test_save = [["k43","k42","k44"],[41,41,41,46,46],41,["点和"]]
    # 大三元
    # 1 test_save = [["k47","g45"],[15,15,31,31,31,46,46,46],46,["自摸"]]
    # 2 test_save = [["k47","k46"],[11,11,21,22,23,45,45,45],11,["点和","和单张"]]
    # 3 test_save = [["k45"],[26,26,26,27,28,46,46,46,47,47,47],47,["点和"]]
    # 4 test_save = [["k42","G46"],[47,47,47,43,43,45,45,45],45,["点和","自风南"]]
    # 绿一色
    # 1 test_save = [["s33"],[32,32,32,33,33,33,34,34,34,36,36],33,["点和"]]
    # 2 test_save = [["k33","G38","k34"],[32,32,32,36,36],36,["自摸","和单张"]]
    # 3 test_save = [["g47","g38","k34"],[32,32,33,33,33],32,["点和"]]
    # 4 test_save = [["s33","k36"],[32,33,34,38,38,38,47,47],34,["点和"]]
    # 5 test_save = [[],[32,32,33,33,34,34,36,36,38,38,47,47,47,47],47,["自摸","和单张"]]
    # 九莲宝灯
    # 1 test_save = [[],[11,11,11,12,13,14,15,16,17,18,19,19,19,18],18,["点和"]]
    # 2 test_save = [[],[21,21,21,22,23,24,25,26,27,28,29,29,29,23],23,["自摸"]]
    # 3 test_save = [[],[31,31,31,32,33,34,35,36,37,38,39,39,39,36],36,["点和"]]
    # 4 test_save = [[],[21,21,21,22,23,24,25,26,27,28,29,29,29,25],25,["自摸"]]
    # 四杠
    # 1 test_save = [["G16","G28","G32","G44"],[29,29],29,["点和"]]
    # 2 test_save = [["G36","g31","G38","G12"],[35,35],35,["自摸"]]
    # 3 test_save = [["G41","g19","g17","G11"],[45,45],45,["自摸"]]
    # 4 test_save = [["g36","G15","g33","g22"],[42,42],42,["点和"]]
    # 5 test_save = [["g43","g25","g13","g21"],[14,14],14,["点和","自风西","场风西"]]
    # 连七对
    # 1 test_save = [[],[12,12,13,13,14,14,15,15,16,16,17,17,18,18],18,["点和"]]
    # 2 test_save = [[],[23,23,24,24,25,25,26,26,27,27,28,28,29,29],25,["自摸"]]
    # 3 test_save = [[],[31,31,32,32,33,33,34,34,35,35,36,36,37,37],33,["点和"]]
    # 十三幺
    # 1 test_save = [[],[11,19,21,29,31,39,41,42,43,44,44,45,46,47],19,["自摸"]] ...
    # 清幺九
    # 1 test_save = [["k29","g19","k11"],[31,31,21,21,21],31,["点和","和单张"]]
    # 2 test_save = [["G39","k21"],[11,11,11,19,19,19,29,29],29,["自摸","和单张"]]
    # 3 test_save = [["k19","g29"],[11,11,11,31,31,39,39,39],39,["自摸"]]
    # 4 test_save = [["k31","k39","k19"],[11,11,11,29,29],11,["点和"]]
    # 5 test_save = [[],[11,11,19,19,31,31,31,31,39,39,21,21,29,29],19,["点和"]]
    # 小四喜
    # 1 test_save = [["k41","k42"],[43,43,43,44,44,47,47,47],47,["点和"]]
    # 2 test_save = [["k29","k44"],[41,41,41,42,42,43,43,43],43,["自摸","场风西","自风东"]]
    # 3 test_save = [["k43","k14","g44"],[41,41,42,42,42],41,["点和","和单张","场风东","自风西"]]
    # 4 test_save = [["k41","k43"],[35,36,37,42,42,44,44,44],44,["点和","场风南","自风南"]]
    # 小三元
    # 1 test_save = [["k46","g39"],[11,12,13,45,45,45,47,47],47,["点和","和单张"]]
    # 2 test_save = [["g47","k27","G46"],[22,22,22,45,45],22,["自摸"]]
    # 3 test_save = [["k42","k47","k45"],[43,43,43,46,46],43,["点和"]]
    # 4 test_save = [[],[43,43,43,44,44,44,45,45,45,47,47,47,46,46],46,["自摸","场风南","自风北","和单张"]]
    # 字一色
    # 1 test_save = [["k44","k46","g41"],[43,43,43,45,45],43,["自摸","场风东","自风北"]]
    # 2 test_save = [["k47","k44","k42"],[41,41,45,45,45],45,["点和","场风西","自风西"]]
    # 3 test_save = [[],[41,41,42,42,43,43,44,44,45,45,46,46,47,47],42,["点和","和单张"]]
    # 四暗刻
    # 1 test_save = [["G12","G23"],[15,15,15,36,36,36,21,21],15,["自摸"]]
    # 2 test_save = [["G18"],[34,34,34,26,26,26,44,44,44,47,47],47,["自摸","场风北","自风西","和单张"]]
    # 3 test_save = [[],[11,11,13,13,13,37,37,37,25,25,25,41,41,41],11,["点和","场风南","自风南"]]
    # 一色双龙会
    # 1 test_save = [[],[11,11,12,12,13,13,15,15,17,17,18,18,19,19],13,["自摸","和单张"]]
    # 2 test_save = [["s38","s32"],[31,32,33,35,35,37,38,39],39,["点和"]]
    # 一色四同顺
    # 1 test_save = [["s13","s13"],[12,12,13,13,14,14,18,18],13,["自摸","和单张"]]
    # 2 test_save = [["s27","s27","s27"],[11,11,26,27,28],28,["点和","和绝张"]]
    # 3 test_save = [["s32"],[31,31,31,32,32,32,33,33,33,44,44],33,["自摸"]]
    # 一色四节高
    # 1 test_save = [["k25","G24"],[22,22,22,23,23,23,28,28],23,["自摸"]]
    # 2 test_save = [["k16","k17"],[15,15,15,18,18,18,31,31],18,["点和"]]
    # 3 test_save = [["k38","k39","g36"],[37,37,37,42,42],37,["点和"]]
    # 一色四步高
    # 1 test_save = [["s34"],[32,33,34,34,35,35,36,36,37,26,26],37,["自摸"]]
    # 2 test_save = [["s18","s12"],[13,14,15,15,16,17,45,45],13,["点和"]]
    # 三杠
    # 1 test_save = [["G31","G44","G12"],[23,23,25,25,25],23,["点和","场风北","自风东"]]
    # 2 test_save = [["G21","g14","G43"],[13,13,13,47,47],13,["自摸","场风东","自风南"]]
    # 3 test_save = [["g45","s26","G46","G36"],[42,42],42,["点和","和单张"]]
    # 4 test_save = [["g34","g22","G15"],[16,17,18,41,41],16,["点和"]]
    # 混幺九
    # 1 test_save = [["g11","g46","k21"],[31,31,43,43,43],43,["点和","场风东","自风西"]]
    # 2 test_save = [["k45","k21","G44"],[29,29,47,47,47],47,["自摸","场风北","自风南"]]
    # 3 test_save = [["k41","k39"],[19,19,21,21,21,42,42,42],21,["点和","场风西","自风北"]]
    # 4 test_save = [[],[11,11,19,19,31,31,29,29,41,41,44,44,46,46],19,["自摸"]]
    # 七对
    # 1 test_save = [[],[13,13,15,15,15,15,31,31,39,39,22,22,27,27],22,["自摸"]]
    # 2 test_save = [[],[14,14,16,16,18,18,32,32,36,36,22,22,28,28],18,["点和"]]
    # 3 test_save = [[],[17,17,19,19,33,33,39,39,24,24,25,25,45,45],39,["点和"]]
    # 七星不靠
    # 1 test_save = [[],[11,14,17,32,35,23,29,41,42,43,44,45,46,47],42,["自摸"]]
    # 2 test_save = [[],[12,15,18,23,26,29,34,41,42,43,44,45,46,47],34,["点和"]]
    # 全双刻
    # 1 test_save = [["G16","k18","k38"],[26,26,26,28,28],26,["点和"]]
    # 2 test_save = [["k26","g24"],[32,32,34,34,34,32,22,22],12,["自摸"]]
    # 3 test_save = [["k36","k14"],[12,12,22,22,28,28,28,12],12,["点和"]]
    # 清一色
    # 1 test_save = [[],[11,11,11,12,13,14,15,16,16,17,18,19,19,19],12,["点和"]]
    # 2 test_save = [["k38"],[32,32,33,33,33,34,34,34,35,36,36],35,["自摸"]]
    # 3 test_save = [["s33","k37","s32"],[38,38,38,39,39],38,["点和"]]
    # 4 test_save = [[],[21,21,22,22,23,23,24,24,25,25,28,28,29,29],22,["自摸"]]
    # 一色三同顺
    # 1 test_save = [["s12","s12"],[11,12,13,17,18,19,42,42],11,["点和"]]
    # 2 test_save = [[],[15,15,15,16,17,25,25,25,26,26,26,27,27,27],17,["自摸"]]
    # 一色三节高
    # 1 test_save = [["k39"],[32,32,32,33,33,33,34,34,34,41,41],33,["点和"]]
    # 2 test_save = [["k14","k13"],[15,15,15,31,31,22,23,24],24,["点和"]]
    # 3 test_save = [["k29","g27","k28"],[17,18,19,19,19],19,["自摸"]]
    # 全大
    # 1 test_save = [["k19","g27"],[18,18,37,37,38,38,39,39],18,["点和","和单张"]]
    # 2 test_save = [[],[17,17,18,18,19,19,38,38,39,39,28,28,29,29],39,["自摸","和单张"]]
    # 全中
    # 1 test_save = [[],[14,14,15,15,16,16,35,35,24,24,25,25,26,26],25,["自摸","和单张"]]
    # 2 test_save = [["k34","k36"],[14,14,14,16,16,24,24,24],14,["点和"]]
    # 3 test_save = [["s15","k34"],[35,35,36,36,36,25,25,25],36,["点和"]]
    # 4 test_save = [[],[16,16,34,34,35,35,36,36,24,24,25,25,26,26],16,["点和"]]
    # 全小
    # 1 test_save = [["s32"],[11,11,12,12,13,13,31,31,21,22,23],13,["点和","和单张"]]
    # 2 test_save = [["k12","g13"],[32,32,32,22,22,23,23,23],23,["自摸"]]
    # 3 test_save = [[],[31,31,31,31,32,32,33,33,21,21,22,22,23,23],31,["自摸"]]
    # 清龙
    # 1 test_save = [[],[11,11,11,11,12,13,14,15,16,17,18,19,19,19],11,["点和"]]
    # 2 test_save = [["s12","s35","s32","s38"],[31,31],31,["点和","和单张"]]
    # 3 test_save = [["s18"],[11,12,13,14,15,15,16,16,17,28,28],16,["自摸"]]
    # 4 test_save = [["g44","s28"],[21,22,23,24,25,26,45,45],25,["自摸","和单张","场风北","自风东"]]
    # 5 test_save = [[],[18,18,31,32,33,34,35,36,37,38,39,28,28,28],36,["点和"]]
    # 6 test_save = [["s22"],[33,33,33,24,25,26,27,28,29,41,41],29,["点和"]]
    # 一色三步高
    # 1 test_save = [[],[11,12,13,13,14,15,15,16,17,18,18,18,19,19],15,["自摸"]]
    # 2 test_save = [["s35"],[14,14,32,33,34,35,36,36,37,37,38],38,["点和"]]
    # 3 test_save = [["g46","s23"],[21,22,23,23,24,25,29,29],23,["点和"]]
    # 全带五
    # 1 test_save = [["s14","s35"],[15,15,23,24,25,25,26,27],15,["点和","和单张"]]
    # 2 test_save = [["G25"],[15,15,15,15,16,17,33,34,35,35,35],34,["自摸"]]
    # 3 test_save = [["s25","s16"],[13,14,15,35,35,35,25,25],13,["点和"]]
    # 三同刻
    # 1 test_save = [["k14","k22"],[12,12,12,32,32,32,34,34],34,["自摸"]]
    # 2 test_save = [["g31","k11"],[39,39,21,21,21,28,28,28],28,["点和"]]
    # 3 test_save = [["k27","k37"],[17,17,17,22,23,24,46,46],17,["点和"]]
    # 三暗刻
    # 1 test_save = [["G22","g17","G31","G46"],[16,16],16,["点和"]]
    # 2 test_save = [["G42","G33","G12"],[29,29,45,45,45],45,["点和","场风东","自风东"]]
    # 3 test_save = [["G43"],[13,13,19,19,19,34,35,35,35,35,36],35,["点和","场风北","自风西"]]
    # 4 test_save = [["s17"],[21,21,26,26,26,27,27,27,28,28,28],28,["自摸"]]
