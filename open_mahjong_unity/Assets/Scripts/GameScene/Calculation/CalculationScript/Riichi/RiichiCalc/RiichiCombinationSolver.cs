...
namespace Riichi {
    /// <summary>
    /// 立直麻将一般型组合解算（4面子+1雀头）与组合映射。
    /// </summary>
    public class RiichiCombinationSolver {
        ...
        public void BuildHandAndCombinationMapping(RiichiPlayerTiles pt) {
        ...
        public void NormalCheck(RiichiPlayerTiles pt, List<RiichiPlayerTiles> doneList) {
            if (pt.CompleteStep == 14) {
                doneList.Add(pt);
                return;
            }
            if (pt.CompleteStep == 0 && !BlockCheck(pt)) return;
            ...
            while (allList.Count > 0) {
                ...
                if (cur.CompleteStep == 14) endList.Add(cur);
            }
        ...
        private bool BlockCheck(RiichiPlayerTiles pt) {
        ...
        private List<RiichiPlayerTiles> TraverseQuetou(RiichiPlayerTiles pt) {
        ...
        private void TraverseKezi(RiichiPlayerTiles pt, List<RiichiPlayerTiles> allList) {
            int prevId = 0;
            foreach (int t in pt.HandTiles) {
        ...
        private void TraverseDazi(RiichiPlayerTiles pt, List<RiichiPlayerTiles> allList) {
            int prevId = 0;
            foreach (int t in pt.HandTiles) {
