...
模型源文件位于相邻 `../model/`；准确尺寸与闭合网格说明以该目录 README 为准。旧平面环 PNG 与 **256px 缩略图**继续服务原设置面板，原橙色选中框不变。它们不是本 PSD 的展开图集；自定义平面边框仍沿用原回退路径。
...
主项目提供 **18 个样式，共用 17 张原底纹**。顺序为原版橙木（默认）、深色橙木、5 款 Relief 材质设计，最后是 10 款 Focus 纯色及原版蓝色。旧选择名称继续可用；只调整显示顺序，不重置已保存的玩家选择。
