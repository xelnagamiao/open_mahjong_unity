...
Python 服务端是南雀麻将的权威实现。客户端计算只服务于即时提示，不参与最终合法性和结算决策。
...
- Unity 继续使用既有 `broadcast_hand_action -> AskHandAction -> ShowCurrentPlayer` 路径刷新行动方和中央余牌，不需要南雀专用客户端刷新函数。
...
- 排名、和牌次数、放铳、和牌巡目、副露和稳定番种 ID 在整场结束时写入南雀专用统计表。
...
未修改 `Game3DManager*`、`NormalGameStateManager*`、`RoundEndPresentation*`、结果面板或牌谱主流程；计分板仅补充南雀静态映射与番数显示格式，统计只进入数据路由和玩家资料页。
