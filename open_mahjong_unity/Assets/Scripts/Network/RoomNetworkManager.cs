...
public class RoomNetworkManager : MonoBehaviour {
    ...
    public async void Create_Changsha_Room(Changsha_Create_RoomConfig config) {
        ...
        try {
            ...
            Debug.Log($"发送创建长沙麻将房间消息: {config.RoomName}, {config.GameRound}, {config.SubRule}, open_kong={config.OpenKongReplacementCount}, bird_count={config.BirdCount}, dealer_bird={config.DealerBird}, base_no_dealer={config.BaseScoreNoDealer}");
            await GetWebSocket().SendText(JsonConvert.SerializeObject(request));
        } catch (Exception e) {
            NetworkManager.Instance.CreateRoomResponse.Invoke(false, e.Message);
        }
